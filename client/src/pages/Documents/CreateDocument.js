...
function CreateDocument({ manual }) {
    ...
    const uploadToIpfs = async (data) => {
        ...
        try {
            await axiosInstance.post('transactions/ipfs', data, { headers: { 'Content-Type': 'multipart/form-data' } }).then((response) => {
                path = response.data;
    ...
    const mintDocument = async (wallet, path) => {
        ...
        try {
            const _receiver = manual ? form.memberAddress : request.requestor.walletAddress;
            ...
            //     .then((gasEstimate) => contract.sendDocument(_receiver, _type, _uri, _docId, { value: opts.value.add(gasEstimate) }))
            //     .then((response) => {
    ...
    const ProcessDocument = async (file) => {
        ...
        if (!manual) formData.append('requestId', id);
        if (manual) formData.append('body', form);
                ...
                <section id="CreateDocument">
                    <div id="CreateDocument__Container">
                        {!file ? (
                            ...
                            <div>
                                <div id="uploadDocument__ViewUploaded__Container">{file && <img src={URL.createObjectURL(file)} alt="uploaded" id="CreateDocument__Image" />}</div>
                        ...
                        {manual ? (
                            <>
                                ...
                                <div className="SidePanel__Date">
                                    ...
                                    />
                                    <TextField id="outlined-search" label="Document Title" type="text" required onChange={(e) => setForm({ ...form, docTitle: e.target.value })} />
                                    ...
                                    <div id="SidePanel__Buttons">
                                        <Button variant="outlined" onClick={() => navigate('/documents/requests/toprocess')}>
                                            Cancel
                                        </Button>
                                        <Button variant="contained" onClick={() => ProcessDocument(file)}>
                                            Process
                                        </Button>
                                    </div>
                                </div>
                            ...
                            <>
                                <div id="SidePanel__Requestor">
                                    ...
                                    <Chip
                                        id="SidePanel__Requestor__Chip"
                                        label={ShortingWallet(request.requestor.walletAddress)}
                                        variant="outlined"
                                        onClick={() => {
                                            navigate(`/users/${request.requestor.walletAddress}`);
                                        }}
                                    />
                                </div>
                                <div className="SidePanel__Date">
                                ...
                                {!request.details.proof ? (
                                    ...
                                    <div className="SidePanel__Date">
                                        ...
                                        <Button id="ProofofPaymentButton" href={request.details.proof} target="_blank" variant="contained">
                                            view image
                                        </Button>
                                    </div>
                                )}
